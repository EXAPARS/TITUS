...
#ifndef __DLB_LOGGER_H__
#define __DLB_LOGGER_H__
...
#define DLB_LOGGER_THEFT_BUFFER_SIZE (sizeof(theft_info) * 100)

#define DLB_LOGGER_BUFFERED_THEFT_DUMP



struct theft_info{
    theft_info() {}
    theft_info(size_t theft_id, size_t context_id, size_t session_id, uint64_t start_date, uint64_t end_date, gaspi_rank_t thief, gaspi_rank_t victim, gaspi_rank_t tasks_owner, dlb_int theft_status, size_t nb_task_theft):
        theft_id(theft_id),
...
struct DLB_session_info{
    ...
    DLB_session_info & operator+=(DLB_session_info & arg);
    void print(std::ostream & out = std::cout)const;
};
static inline std::ostream & operator<<(std::ostream & out, const DLB_session_info & arg) {
    arg.print(out);
    return out;
}
...
class DLB_Logger_impl{
    DLB_Context_impl * context;
    
    std::list<DLB_session_info*> sessions;
    DLB_session_info * current_session;
    DLB_session_info agregated_sessions;
...
public:
    DLB_Logger_impl(DLB_Context_impl * arg, bool log_all = true, size_t local_buffer_size_in_bytes = DLB_LOGGER_THEFT_BUFFER_SIZE,bool use_rdtsc = true, bool autodump = false);
    ~DLB_Logger_impl() {
    }
    ...
    void print_all_sessions(std::ostream & out = std::cout) const {
        for (std::list<DLB_session_info*>::const_iterator it = sessions.begin(); it != sessions.end() ; it ++){
            (*it)->print(out);
            out << std::endl;
		}
    }
    ...
    void set_autodump(bool val, std::ostream & output = TITUS_DBG);
    void set_autodump(bool val, const char *logdir);
    
    // notification types (in DLB)
    
    // INIT AND WORK SPAWNING
    // debut de session de travail (DLB_work)
    void signal_start_DLB_session();
    void signal_end_DLB_session();
    ...
    // cible vol
    // nb tache volées
    void signal_end_theft(dlb_int theft_status, size_t nb_task_theft, dlb_int first_task_id, gaspi_rank_t target_rank, gaspi_rank_t owner);
